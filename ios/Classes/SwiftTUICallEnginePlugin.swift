...
public class SwiftTUICallEnginePlugin: NSObject, FlutterPlugin {
    ...
    public static func register(with registrar: FlutterPluginRegistrar) {
        ...
        registrar.register(PlatformVideoViewFactory(messager: registrar.messenger()), withId: "TUICallEnginVideoView")
    }
...
extension SwiftTUICallEnginePlugin {
    ...
    func setVideoEncoderParams(call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        if let resolutionIndex = paramsDic["resolution"] {
            switch resolutionIndex {
                ...
                case 1:
                    params.resolution = ._640_480

                case 2:
                    params.resolution = ._960_540

                case 3:
                    params.resolution = ._960_720
    ...
    func queryRecentCalls(call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        if let begin = filterDic["begin"] as? Double {
            filter.begin = begin
        }
        if let end = filterDic["end"] as? Double {
            filter.end = end
        }
...
private extension SwiftTUICallEnginePlugin {
